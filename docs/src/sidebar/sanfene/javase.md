...
抽象类是一个不能被实例化的类，但可以包含抽象方法（没有实现体的方法）和具体方法（有实现体的方法）。抽象类可以包含成员变量、构造方法、静态方法等。使用 abstract 关键字定义。

是一种完全抽象的类型，不能包含具体方法的实现（Java 8 之前），但从 Java 8 开始可以包含默认方法（default）和静态方法。接口只能包含常量（默认是 public static final）和抽象方法（默认是 public abstract）。使用 interface 关键字定义。
